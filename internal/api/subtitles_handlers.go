...
// handleTitleSubtitle serves one text Subtitle track of a Title as WebVTT. It
// resolves the Title under the caller's scope (an out-of-scope / unknown Title is
// 404, hide existence), finds the track by id across the embedded Streams and the
// Sidecar/Fetched rows, and produces WebVTT:
//   - sidecar/fetched TEXT → read the on-disk file, convert in-process;
//   - embedded TEXT        → ffmpeg-extract the Stream to WebVTT;
//   - image (either source), an unknown id, or an unconvertible text format → 404.
//
// The response is cacheable (the conversion is deterministic for a given track)
// and served as text/vtt.
func handleTitleSubtitle(cat *catalog.Service, titleID, subID string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		vtt, err := subtitleVTT(r.Context(), d, subID)
		switch {
		case errors.Is(err, errSubtitleNotText), errors.Is(err, errSubtitleNotFound):
			// An unknown id, an image track, or a text format we can't convert are all
			// "no text subtitle here" — hide the distinction behind a 404.
			writeError(w, http.StatusNotFound, codeNotFound, "subtitle not found", nil)
			return
		case err != nil:
		...
		// The converted bytes are deterministic for a given track (path/mtime don't
		// change without a rescan), so let the browser cache the <track> fetch.
...
// dispatchTitleSubtitle routes GET {id}/subtitles/{subId}.vtt off the title
// subtree. It is a media GET (cookie-or-bearer) like the artwork leaf. The subId
// is the path element before the .vtt suffix and must be a single element.
func dispatchTitleSubtitle(deps Deps, rest string) (http.HandlerFunc, bool) {
	...
	subID, ok := strings.CutSuffix(tail, ".vtt")
	if !ok || subID == "" || strings.Contains(subID, "/") || strings.Contains(titleID, "/") {
		return func(w http.ResponseWriter, r *http.Request) {
			writeError(w, http.StatusNotFound, codeNotFound, "resource not found", nil)
		}, true
	}
	return requireMethod(http.MethodGet,
		requireAuthAllowCookie(deps.Auth, requireScope(deps.Access, handleTitleSubtitle(deps.Catalog, titleID, subID)))), true
}
