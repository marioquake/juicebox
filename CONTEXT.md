...
**Subtitle track**:
The client-facing, selectable subtitle a viewer turns on for a Title — the union of three **sources**: an embedded subtitle Stream, a Sidecar subtitle, and a Fetched subtitle (downloaded from a provider). Always source-tagged, because the source decides delivery ([ADR-0020](./docs/adr/0020-subtitle-delivery-in-band-hls-out-of-band-track-image-burn-in.md)): text tracks (embedded text, text sidecars, fetched) are selectable and switch instantly — served as WebVTT, or in their original format (ASS styling intact) to a client whose Capability profile declares support ([ADR-0033](./docs/adr/0033-original-format-subtitle-delivery-negotiated-by-capability.md)); image tracks (embedded PGS/VOBSUB) burn in on explicit selection and escalate the playback tier. Distinct from the embedded Stream (one possible source) — a Subtitle track may have no Stream at all.
...
**Targeted scan**:
A Scanner run restricted to the folders of a single browsable entity — triggered from that entity's detail page (Movie, Show, Album, Artist) to reflect local changes to just that entity without re-walking the whole Library ([ADR-0030](./docs/adr/0030-targeted-scan-scope-from-existing-file-folders.md)). Its scope is the set of folders currently holding the entity's Files (a Movie's folder or bare file, a Show's folder, the parent folders of an Album's or Artist's Tracks). Within that scope it behaves exactly like a full scan — same identity derivation ([ADR-0002](./docs/adr/0002-naming-convention-is-identity-authority.md)), Match-override preservation, and soft-delete of Missing Files ([ADR-0008](./docs/adr/0008-incremental-scan-soft-delete-missing-files.md)) — only narrower and per-folder: a File gone from a folder the scan did reach is marked Missing (and a Title whose Files are all Missing is hidden), while a folder it cannot reach is left untouched rather than assumed empty ([ADR-0031](./docs/adr/0031-targeted-scan-per-folder-soft-delete-and-shared-lock.md)). Because the scope names each folder, a shared folder is walked whole, so a Targeted scan launched from one entity can legitimately file media belonging to a sibling entity in the same folder. Does not discover folder renames, moves, or new folders (a full scan's job), and auto-enriches only the Titles it touched.
_Avoid_: Title scan (the scope is a browsable entity, which for a Show or Artist spans many Titles, not one), Rescan, Quick scan, Partial scan.
